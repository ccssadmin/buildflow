              ...
              <div className="d-flex justify-content-between flex-grow-1 mx-2">
                {weekDates.map((date, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`weekly-date ${isSelected ? 'selected-weekly-date' : ''
                        }`}
                      onClick={() => {
          ...
          <Card key={notification.id} className="mb-3 border rounded-4">
            <div className="card-pending-approvel">
              <div className="card-pending-approvel-header d-flex justify-content-between align-items-start">
                {/* Inside the notification card rendering */}
                <div className="card-pending-approvel-project-title">
                  <span
                    className="project-dept"
                    style={{
                      backgroundColor: notification.department === 'Finance' ? '#FF4040' :
                        notification.department === 'Purchase Team' ? '#4086FF' :
                          'inherit'
                    }}
                  >
                  ...
                  {week.map((day, dayIndex) => {
                    ...
                    return (
                      <Col key={dayIndex}>
                        {day && (
                          <div
                            className={`p-2 rounded-circle position-relative ${isSelected ? 'bg-warning bg-opacity-25' :
          ...
          {calendarViewMode === 'year' && (
            <Row className="g-2">
              {monthNames.map((month, index) => (
                <Col key={index} xs={4}>
                  <div
                    className={`p-3 text-center rounded ${currentDate.getMonth() === index ? 'bg-warning bg-opacity-25' : 'bg-light'
                      }`}
                    style={{ cursor: 'pointer' }}
          ...
          {calendarViewMode === 'yearList' && (
            <Row className="g-2">
              {yearList.map((year) => (
                <Col key={year} xs={4}>
                  <div
                    className={`p-3 text-center rounded ${currentDate.getFullYear() === year ? 'bg-warning bg-opacity-25' : 'bg-light'
                      }`}
                    style={{ cursor: 'pointer' }}
